...
class InsightRepo:
    ...
    def create_user(self, username: str, password: str) -> str:
        """
        Create a new user (simple fake auth for demo).
        
        Args:
            username: Unique username
            password: Plain text password (no hashing for demo)
        ...
        data = {
            ...
            "password": password.strip(),  # Plain text for demo
        }
        
        try:
            response = self.supabase.table("if_users").insert(data).execute()
            if response.data:
    ...
    def get_user_by_username(self, username: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            response = self.supabase.table("if_users")\
                .select("*")\
    ...
    def verify_user(self, username: str, password: str) -> Optional[str]:
        """
        Verify user credentials (simple plain text comparison for demo).
        ...
        if user.get("password") == password.strip():
            user_id = str(user["user_id"])
            # Update last_login
            try:
                self.supabase.table("if_users")\
